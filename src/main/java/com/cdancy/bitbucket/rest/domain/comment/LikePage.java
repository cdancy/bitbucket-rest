...
@AutoValue
public abstract class LikePage implements Page<Author>, ErrorsHolder {

    @SerializedNames({ "start", "limit", "size", 
            "nextPageStart", "isLastPage", "values", "errors" })
    public static LikePage create(final int start,
                                  final int limit,
                                  final int size,
                                  final int nextPageStart,
                                  final boolean isLastPage,
                                  @Nullable final List<Author> values,
                                  @Nullable final List<Error> errors) {
        
        return new AutoValue_LikePage(start,
                limit, 
                size, 
                nextPageStart, 
                isLastPage,
