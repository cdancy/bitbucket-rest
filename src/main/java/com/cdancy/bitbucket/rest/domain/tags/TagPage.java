...
@AutoValue
public abstract class TagPage implements Page<Tag>, ErrorsHolder {

    @SerializedNames({ "start", "limit", "size", 
            "nextPageStart", "isLastPage", "values", "errors" })
    public static TagPage create(final int start, 
            final int limit, 
            final int size, 
            final int nextPageStart, 
            final boolean isLastPage,
            @Nullable final List<Tag> values, 
            @Nullable final List<Error> errors) {
        
        return new AutoValue_TagPage(start, 
                limit, 
                size, 
                nextPageStart, 
                isLastPage,
