...
@Test(groups = "live", testName = "HookSettingsDevTest", singleThreaded = true)
public class HookSettingsDevTest extends BaseBitbucketApiLiveTest {
    ...
    public void testGetHook() {
        final Hook hook = api().getHook(projectKey, repoKey, hookKey.details().key());
        assertThat(hook).isNotNull();
        assertThat(hook.errors()).isEmpty();
        assertThat(hook.details().key().equals(hookKey.details().key())).isTrue();
        assertThat(hook.enabled()).isFalse();
    }

    @Test(dependsOnMethods = {"testGetHook"})
    public void testGetHookSettings() {
        ...
        assertThat(hookPage.size()).isGreaterThan(0);
        for (final Hook hook : hookPage.values()) {
            System.out.println(">>>>>>>>>>>>>>> FOUND HOOK: " + hook.details().key());
            try {
                ...
                if (hookUpdate.errors().size() > 0) {
                    System.out.println("@@@@@@@@ WE HAVE ERRORS: " + hookUpdate);
                } else {
                    System.out.println("@@@@@@@@@@ NO ERRORS: " + hookUpdate);
                }
                System.out.println("++++++++++++ BODY: " + hooks);
            } catch (Exception e) {
        ...
        /*
        final HookSettings hookSettings = api().getHookSettings(projectKey, repoKey, hookKey.details().key());
        assertThat(hookSettings).isNotNull();
        assertThat(hookSettings.errors()).isEmpty();
        assertThat(hookSettings.settings()).isNotNull();
*/
    }
