...
public class PostWebHookApiMockTest extends BaseBitbucketMockTest {
    ...
    public void testGetPostWebHooks() throws Exception {
        ...
        try {
            final List<PostWebHook> postWebhookList = api.list(projectKey, repoKey);
            assertEquals(postWebhookList.get(0).branchCreated(), false);
            assertEquals(postWebhookList.get(0).branchDeleted(), false);
            assertEquals(postWebhookList.get(0).enabled(), true);
            assertEquals(postWebhookList.get(0).prCommented(), false);
            assertEquals(postWebhookList.get(0).prCreated(), false);
            assertEquals(postWebhookList.get(0).prDeclined(), false);
            assertEquals(postWebhookList.get(0).prMerged(), false);
            assertEquals(postWebhookList.get(0).prReopened(), false);
            assertEquals(postWebhookList.get(0).prRescoped(), false);
            assertEquals(postWebhookList.get(0).prUpdated(), false);
            assertEquals(postWebhookList.get(0).repoMirrorSynced(), false);
            assertEquals(postWebhookList.get(0).repoPush(), false);
            assertEquals(postWebhookList.get(0).tagCreated(), true);
            assertEquals(postWebhookList.get(0).title(), "test");
            assertEquals(postWebhookList.get(0).url(), "test");

            assertEquals(postWebhookList.get(1).branchCreated(), true);
            assertEquals(postWebhookList.get(1).branchDeleted(), true);
            assertEquals(postWebhookList.get(1).enabled(), false);
            assertEquals(postWebhookList.get(1).prCommented(), false);
            assertEquals(postWebhookList.get(1).prCreated(), true);
            assertEquals(postWebhookList.get(1).prDeclined(), true);
            assertEquals(postWebhookList.get(1).prMerged(), true);
            assertEquals(postWebhookList.get(1).prReopened(), true);
            assertEquals(postWebhookList.get(1).prRescoped(), true);
            assertEquals(postWebhookList.get(1).prUpdated(), true);
            assertEquals(postWebhookList.get(1).repoMirrorSynced(), false);
            assertEquals(postWebhookList.get(1).repoPush(), true);
            assertEquals(postWebhookList.get(1).tagCreated(), true);
            assertEquals(postWebhookList.get(1).title(), "http://jenkins.example.com");
            assertEquals(postWebhookList.get(1).url(), "http://jenkins.example.com/bitbucket-scmsource-hook/notify");
